...
sql_worklocation_info_inactives = ''' 
with current_file as (select distinct file from org_lumen.barg order by file desc limit 1)
select id, 'Inactive' as status from org_lumen.ab_worklocations where id not in 
(select distinct w.id from org_lumen.ab_worklocations w
join org_lumen.barg s on concat(s.workstreet, concat(', ', concat(s.workcity, concat(', ', s.workstate_acronym)))) = upper(w.name) 
where file = (select * from current_file) and unionname = 'CWA') '''

df = create_civis_df(sql_worklocation_info_inactives)
error_list = []
count = 0
...
if df.empty:
    ...
    update_table = ''' update org_lumen.ab_worklocations 
                        ...
                        select id from org_lumen.ab_worklocations where id not in 
                        (select distinct w.id from org_lumen.ab_worklocations w
                        join org_lumen.barg s on concat(s.workstreet, concat(', ', concat(s.workcity, concat(', ', s.workstate_acronym)))) = upper(w.name) 
                        where file = (select * from current_file) and unionname = 'CWA')) '''
